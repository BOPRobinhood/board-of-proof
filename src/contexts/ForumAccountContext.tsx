...
export function ForumAccountProvider({ children }: { children: ReactNode }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (r.status === 403 && j.banned) {
        try {
          localStorage.removeItem('bop_forum_profile');
        } catch {
          /* ignore */
        }
  ...
  useEffect(() => {
    ...
    window.addEventListener(BOP_PROFILE_UPDATED_EVENT, onUp);
    return () => window.removeEventListener(BOP_PROFILE_UPDATED_EVENT, onUp);
  }, [refresh]);
