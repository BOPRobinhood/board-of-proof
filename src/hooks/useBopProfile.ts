...
export const BOP_PROFILE_UPDATED_EVENT = 'bop-profile-updated';

function getStoredUsername(wallet: string | null): string | null {
  if (!wallet || typeof window === 'undefined') return null;
  try {
    const raw = localStorage.getItem('bop_forum_profile');
    if (!raw) return null;
...
export function useBopProfile() {
  ...
  useEffect(() => {
    ...
    window.addEventListener(BOP_PROFILE_UPDATED_EVENT, bump);
    return () => {
      window.removeEventListener('focus', bump);
      window.removeEventListener(BOP_PROFILE_UPDATED_EVENT, bump);
    };
  }, []);
