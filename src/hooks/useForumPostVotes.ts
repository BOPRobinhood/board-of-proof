...
function voteApiErrorMessage(status: number, bodyError: string | undefined, fallback: string): string {
  ...
  if (status === 404 || e === 'Not found') {
    return 'Vote API not found on this server. Stop and restart the BOP API from this project (npm run dev, or npm run dev:server) so it loads the latest server/index.mjs. If .env sets VITE_API_BASE, it must point at that same updated API.';
  }
  return e || fallback;
}
