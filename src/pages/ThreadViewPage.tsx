        ...
        {!loading && !loadError && threadMeta ? (
          <div className="forum-thread border border-gray-400 mb-4">
            ...
            {threadMeta.onchain_tx_sig ? (
              ...
              >
                Creation TX:{' '}
                {explorerTxUrl(threadMeta.onchain_tx_sig) ? (
                  <a
                    href={explorerTxUrl(threadMeta.onchain_tx_sig)}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-blue-800 underline hover:text-blue-950 font-mono"
                    title={threadMeta.onchain_tx_sig}
                  >
                    {threadMeta.onchain_tx_sig.slice(0, 10)}…
                    {threadMeta.onchain_tx_sig.slice(-8)}
                  </a>
                ) : (
                  <span className="font-mono" title={threadMeta.onchain_tx_sig}>
                    {threadMeta.onchain_tx_sig.slice(0, 10)}…
                    {threadMeta.onchain_tx_sig.slice(-8)}
                  </span>
                )}
              ...
              return (
                ...
                >
                  <ThreadPost
                    ...
                    isGovernanceBoard={board?.section === 'BOP GOVERNANCE'}
