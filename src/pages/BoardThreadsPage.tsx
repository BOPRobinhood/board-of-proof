                      ...
                      <td className="p-2 border border-gray-400">
                        {threadHref ? (
                          <div className="flex flex-col gap-1">
                            ...
                            {t.onchain_tx_sig ? (
                              explorerTxUrl(t.onchain_tx_sig) ? (
                                <a
                                  href={explorerTxUrl(t.onchain_tx_sig)}
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  className="text-[0.7rem] text-gray-700 hover:text-gray-900 underline font-mono"
                                  title={t.onchain_tx_sig}
                                >
                                  Creation TX: {t.onchain_tx_sig.slice(0, 10)}…
                                  {t.onchain_tx_sig.slice(-8)}
                                </a>
                              ) : (
                                <span
                                  className="text-[0.7rem] text-gray-700 font-mono"
                                  title={t.onchain_tx_sig}
                                >
                                  Creation TX: {t.onchain_tx_sig.slice(0, 10)}…
                                  {t.onchain_tx_sig.slice(-8)}
        ...
        {canStartThread && board ? (
          ...
          >
            <p className="text-sm text-gray-800 m-0 mb-2" style={{ fontFamily: 'Times New Roman, serif' }}>
              <strong>New thread</strong> — title + opening post; MetaMask signs the full message.
            </p>
            <input
              type="text"
              value={newTitle}
