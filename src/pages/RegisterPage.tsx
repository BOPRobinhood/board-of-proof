...
import { BOP_PROFILE_UPDATED_EVENT } from '../hooks/useBopProfile';
...
const RegisterPage = () => {
  ...
  const handleSign = useCallback(async () => {
    ...
    try {
      const msg = [
        'BOP forum registration',
        `Wallet: ${publicKey}`,
        `Nonce: ${nonce}`,
        'By signing, you prove control of this wallet. Username will be linked in the database.',
      ].join('\n');
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      window.dispatchEvent(new Event(BOP_PROFILE_UPDATED_EVENT));
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900">
      <div className="max-w-xl mx-auto px-6 py-8">
        ...
        >
          <p className="m-0">
            BOP does <strong>not</strong> use passwords or traditional login secrets. The project is{' '}
            <strong>partly built on-chain</strong>: you prove control of an EVM wallet by signing a short
          ...
          <p className="m-0">
            ...
            username later — keep your wallet access safe. Below: connect MetaMask, sign, then pick a
            username; availability and signup go through the local API (database on the server).
          </p>
        </div>
              ...
              <div className="text-xs mt-3 text-gray-700 flex-1" style={{ fontFamily: 'Times New Roman, serif' }}>
                ...
                {signComplete && usernameStatus === 'reserved' && (
                  <span className="text-red-800">
                    Reserved by site policy (not your database). Names like admin, moderator, bop, ligder,
