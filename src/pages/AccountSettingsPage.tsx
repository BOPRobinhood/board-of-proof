...
import { BOP_PROFILE_UPDATED_EVENT, useBopProfile } from '../hooks/useBopProfile';
...
const AccountSettingsPage = () => {
    ...
    void (async () => {
      try {
        ...
        if (!r.ok) {
          ...
          throw new Error(msg || 'Could not load BOP balance');
        }
        setLiveLiteUi(
        ...
        if (!cancelled) {
          setLiteError(e instanceof Error ? e.message : 'Could not load BOP balance');
          setLiveLiteUi(null);
        }
      } finally {
  ...
  const uploadAvatarFile = async () => {
    ...
    try {
      ...
      const message = ['BOP avatar upload', `Wallet: ${publicKey}`, `Nonce: ${nonce}`].join('\n');
    ...
    <div className="min-h-screen bg-white text-gray-900">
      <div className="max-w-xl mx-auto px-6 py-8">
        ...
        <p className="text-sm text-gray-700 mb-6" style={{ fontFamily: 'Times New Roman, serif' }}>
          ...
          features go live; $BOP balance is read from the chain for your connected wallet (no transaction).
        </p>
            ...
            <ul className="space-y-3 mb-8 list-none m-0 p-0" style={{ fontFamily: 'Times New Roman, serif' }}>
              ...
              <li className="border border-gray-200 px-3 py-2 bg-gray-50">
                <div className="flex flex-wrap justify-between gap-2">
                  <span className="text-gray-700">$BOP holdings (on-chain)</span>
                  <span className="font-mono text-gray-900 font-semibold">
                    {liteLoading
                      ? '…'
                      : liteError
