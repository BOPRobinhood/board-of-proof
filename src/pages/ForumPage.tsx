...
const OFFICIAL_SECTION_DB = 'BOP OFFICIAL';
const OFFICIAL_SECTION_LABEL = 'BOP Official';
const OFFICIAL_BOARD_BASE = '/forums/bop-official';

const GENERAL_SECTION_DB = 'BOP GENERAL';
const GENERAL_SECTION_LABEL = 'BOP General';
const GENERAL_BOARD_BASE = '/forums/bop-general';

const TECHNICAL_SECTION_DB = 'BOP TECHNICAL';
const TECHNICAL_SECTION_LABEL = 'BOP Technical';
const TECHNICAL_BOARD_BASE = '/forums/bop-technical';
const GOVERNANCE_SECTION_DB = 'BOP GOVERNANCE';
const GOVERNANCE_SECTION_LABEL = 'BOP Governance';
const GOVERNANCE_BOARD_BASE = '/forums/bop-governance';
...
const FORUM_BOARDS_CACHE_VERSION = 2;
...
const ForumPage = () => {
  ...
  const { username, isRegistered, profileLoading } = useBopProfile();
  ...
  const [officialBoards, setOfficialBoards] = useState<ForumBoardRow[]>([]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (raw) {
        ...
        if (
          ...
          setOfficialBoards(allCached.filter((b) => b.section === OFFICIAL_SECTION_DB));
    ...
    void fetch(apiUrl(`/api/forum/boards${walletQ}`))
      .then(async (r) => {
        ...
        setOfficialBoards(all.filter((b) => b.section === OFFICIAL_SECTION_DB));
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900">
      ...
      <div className="max-w-5xl mx-auto px-4 py-5 sm:px-6 sm:py-6">
        ...
        <h1
          className="bop-pixel-title text-center mb-4"
        ...
        <p
          ...
          style={{ fontFamily: 'Arial, sans-serif' }}
        >
          Liteboards are mini-forums for <strong>Robinhood Chain</strong> ERC-20 tokens.
        </p>
        ...
        {!boardsLoading && !boardsError ? (
          <>
            <h2
              ...
              style={{ fontFamily: 'Arial, sans-serif' }}
            >
              <span>{OFFICIAL_SECTION_LABEL}</span>
            </h2>
            {officialBoards.length > 0 ? (
              <ForumBoardsTable boards={officialBoards} boardLinkBase={OFFICIAL_BOARD_BASE} />
            ) : (
