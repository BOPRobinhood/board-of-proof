...
type DecodeResponse = {
  ...
  explorerCluster?: 'mainnet' | 'devnet';
  error?: string;
};
...
export function MemoTxDecoder({ variant = 'default' }: MemoTxDecoderProps) {
  ...
  const decode = async () => {
    const s = sig.trim();
    if (!s) {
      setErr('Paste a transaction hash (0x…).');
      return;
    }
    setLoading(true);
    setErr(null);
    ...
    try {
      const r = await fetch(
        apiUrl(`/api/forum/decode-memo-tx?signature=${encodeURIComponent(s)}`)
      );
  ...
  return (
    <section className={sectionClass} style={{ fontFamily: 'Arial, sans-serif' }}>
      <h2
        className={
          isArchive
            ? 'bop-pixel-title m-0 mb-4 text-center text-gray-900'
            : 'm-0 mb-2 text-lg font-bold text-gray-900'
        }
        style={isArchive ? { fontSize: 'clamp(1.5rem, 4vw, 2.25rem)' } : undefined}
      >
        Verify attestation (Robinhood Chain)
      </h2>
      <p className="text-sm text-gray-700 m-0 mb-2" style={{ fontFamily: 'Times New Roman, serif' }}>
        Paste a Robinhood Chain <strong>transaction hash</strong> (<code className="text-xs">0x…</code>) from
        rh-scan / Blockscout. We decode the <code className="text-xs">attest()</code> call /{' '}
        <code className="text-xs">ForumAttested</code> event and show each pipe-separated field. Message text is
        not on-chain — only hashes and IDs; post body is loaded from the forum by post UUID when applicable.
      </p>
      ...
      >
        <div>
          ...
          <p className="text-gray-800 m-0 flex flex-wrap gap-x-2 gap-y-1 items-baseline">
            {hasDecoded && data && explorerTxUrl(data.signature) ? (
              <a
                href={explorerTxUrl(data.signature)}
                target="_blank"
            ...
            <span className="text-gray-600">
              {hasDecoded && data?.slot != null ? (
                <>· {data.chain === 'solana' ? 'slot' : 'block'} {data.slot}</>
              ) : (
                <>· block {EMPTY_VALUE}</>
              )}
            </span>
        ...
        <div className="border border-gray-300 bg-white p-3">
          <p className="text-xs font-semibold text-gray-700 m-0 mb-2">Attestation payload #1</p>
          ...
          {hasDecoded && data && data.memos.length === 0 ? (
            <p className="text-gray-700 m-0 mb-2" style={{ fontFamily: 'Times New Roman, serif' }}>
              No forum attestation memo found in this transaction.
            </p>
          ) : null}
