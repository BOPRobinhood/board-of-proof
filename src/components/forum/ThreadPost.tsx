...
import { explorerAddressUrl, explorerTxUrl, truncateWalletDisplay } from '../../lib/explorer';
...
type ThreadPostProps = {
  ...
  /** Board section is BOP GOVERNANCE — poll copy + server gates */
...
export function ThreadPost({
  ...
  const walletHref = post.authorWallet ? explorerAddressUrl(post.authorWallet) : '';
  const walletDisplay = post.authorWallet ? (
    walletHref ? (
      ...
      <span
        className="font-mono text-[0.65rem] leading-tight text-gray-800"
        title={post.authorWallet}
      >
        {truncateWalletDisplay(post.authorWallet)}
      </span>
      ...
      >
        ...
        {post.onchainTxSig ? (
          ...
          >
            ...
            {explorerTxUrl(post.onchainTxSig) ? (
              <a
                href={explorerTxUrl(post.onchainTxSig)}
                target="_blank"
                rel="noopener noreferrer"
              ...
              <span className="font-mono" title={post.onchainTxSig}>
                {post.onchainTxSig.slice(0, 10)}…{post.onchainTxSig.slice(-8)}
              </span>
            )}
          </div>
