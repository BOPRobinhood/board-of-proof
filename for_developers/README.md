...
This folder is the **single place** for material that helps people **fork, deploy, audit, and extend** BOP without digging through the whole repo. We intend to keep it accurate as the project evolves—if something here drifts from `main`, treat that as a bug to fix in the same PR as the code change.
...
| Layer | Tech | Notes |
|-------|------|--------|
| **Frontend** | Vite + React + TypeScript + Tailwind (`src/`) | React Router; wallet via MetaMask (client context). |
...
| **On-chain** | Robinhood Chain attestations | Server relays compact attestation payloads with a configured fee-payer; payloads contain hashes + IDs, not full post bodies. |
...
| `/forums` | Forum hub (BOP Official section, boards table). |
...
| `/forums/bop-official` | Section landing. |
| `/forums/bop-official/:boardSlug` | Threads for a board. |
| `/forums/bop-official/:boardSlug/:threadNumber` | Thread view (posts, votes, replies). |
...
- `POST /api/register` — MetaMask-signed registration message
...
- `GET /api/forum/decode-memo-tx` — fetch tx from RPC(s), parse BOP `v1|tc|…` / `v1|rp|…` / `v1|pv|…` / `v1|pm|…` memos
...
| `BOP_FORUM_ATTESTATION_ADDRESS` | Forum attestation contract |
| `ATTESTATION_RELAYER_PRIVATE_KEY` | Relayer key for on-chain attestations (server-only) |
...
```bash
npm install
# Copy .env.example to .env and fill in Supabase + Robinhood Chain / attestation keys
npm run dev
```
...
4. Optional but expected for forums v2: configure Robinhood RPC + attestation relayer for on-chain events.
