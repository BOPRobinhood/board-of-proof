...
function readProjectPublicJson() {
  try {
...
function readProjectPublicToken() {
  const data = readProjectPublicJson();
  const v = data?.bopTokenAddress ?? data?.liteTokenMint;
  return typeof v === 'string' ? v.trim() : '';
}

function readProjectPublicRewardNft() {
  const data = readProjectPublicJson();
  const v = data?.bopRewardNftAddress;
  return typeof v === 'string' ? v.trim() : '';
}
...
/** $BOP ERC-20 contract address (0x…). Legacy env: LITE_TOKEN_MINT. */
const BOP_TOKEN_ADDRESS =
  process.env.BOP_TOKEN_ADDRESS?.trim() ||
  process.env.LITE_TOKEN_MINT?.trim() ||
  readProjectPublicToken();
/** Alias for leftover Solana dividend/memo paths that still reference the old name. */
const LITE_TOKEN_MINT = BOP_TOKEN_ADDRESS;

/** BopRewardNft (400k $BOP ↔ 1 NFT). Env wins over project.public.json. */
const BOP_REWARD_NFT_ADDRESS =
  process.env.BOP_REWARD_NFT_ADDRESS?.trim() || readProjectPublicRewardNft();

const REWARD_NFT_IFACE = new Interface([
  'event Transfer(address indexed from, address indexed to, uint256 indexed tokenId)',
  'function balanceOf(address owner) view returns (uint256)',
  'function totalMinted() view returns (uint256)',
]);
const REWARD_NFT_TRANSFER_TOPIC = REWARD_NFT_IFACE.getEvent('Transfer').topicHash;

/** Robinhood Chain JSON-RPC for $BOP balance reads. Legacy: ETH_RPC_URL, RPC_URL. */
const ROBINHOOD_RPC_URL =
  process.env.ROBINHOOD_RPC_URL?.trim() ||
  process.env.ETH_RPC_URL?.trim() ||
  process.env.RPC_URL?.trim() ||
  '';
/** @deprecated alias — prefer ROBINHOOD_RPC_URL */
const ETH_RPC_URL = ROBINHOOD_RPC_URL;

/**
 * Site owner (EVM address). Always treated as admin — put your MetaMask address here.
 * Does not need is_admin in the DB (we also auto-flag on register / profile).
 */
let OWNER_WALLET = '';
try {
  const rawOwner = process.env.OWNER_WALLET?.trim();
  if (rawOwner) OWNER_WALLET = getAddress(rawOwner);
} catch {
  console.warn('[owner] OWNER_WALLET is set but not a valid EVM address — ignoring');
  OWNER_WALLET = '';
}

/** Legacy Solana RPC — only used by leftover memo / SPL dividend code paths. */
...
const FORUM_ATTESTATION_ABI = [
  'function attest(address author, string memo)',
  'function relayer() view returns (address)',
];
/** Max memo bytes — must match BopForumAttestation.MAX_MEMO_BYTES */
const EVM_ATTESTATION_MAX_MEMO_BYTES = 2048;

const BOP_FORUM_ATTESTATION_ADDRESS =
  process.env.BOP_FORUM_ATTESTATION_ADDRESS?.trim() || '';
const ATTESTATION_RELAYER_PRIVATE_KEY =
  process.env.ATTESTATION_RELAYER_PRIVATE_KEY?.trim() || '';

let ethProvider = null;
function getEthProvider() {
  if (!ETH_RPC_URL) return null;
  if (!ethProvider) ethProvider = new JsonRpcProvider(ETH_RPC_URL);
  return ethProvider;
}

let attestationRelayer = null;
let forumAttestationContract = null;
try {
  if (
    ROBINHOOD_RPC_URL &&
    BOP_FORUM_ATTESTATION_ADDRESS &&
    isAddress(BOP_FORUM_ATTESTATION_ADDRESS) &&
    ATTESTATION_RELAYER_PRIVATE_KEY
  ) {
    const provider = getEthProvider();
    attestationRelayer = new Wallet(ATTESTATION_RELAYER_PRIVATE_KEY, provider);
    forumAttestationContract = new Contract(
      getAddress(BOP_FORUM_ATTESTATION_ADDRESS),
      FORUM_ATTESTATION_ABI,
      attestationRelayer
    );
  }
} catch (e) {
  console.warn(
    '[attest] EVM relayer config invalid —',
    e instanceof Error ? e.message : e
  );
  attestationRelayer = null;
  forumAttestationContract = null;
}

function evmAttestationReady() {
  return Boolean(forumAttestationContract && attestationRelayer);
}

function isOwnerWallet(walletOk) {
  return Boolean(OWNER_WALLET && walletOk && OWNER_WALLET === walletOk);
}
...
if (OWNER_WALLET) {
  console.log('[owner] OWNER_WALLET configured:', OWNER_WALLET);
} else {
  console.warn('[owner] OWNER_WALLET unset — no env owner/admin');
}
if (ROBINHOOD_RPC_URL) {
  console.log('[rpc] ROBINHOOD_RPC_URL configured');
} else {
  console.warn('[rpc] ROBINHOOD_RPC_URL unset — $BOP balance reads return 0');
}
if (BOP_TOKEN_ADDRESS) {
  console.log('[bop] BOP_TOKEN_ADDRESS configured:', BOP_TOKEN_ADDRESS);
} else {
  console.warn('[bop] BOP_TOKEN_ADDRESS unset — balance sync disabled');
}
if (BOP_REWARD_NFT_ADDRESS && isAddress(BOP_REWARD_NFT_ADDRESS)) {
  console.log('[nft] BOP_REWARD_NFT_ADDRESS configured:', getAddress(BOP_REWARD_NFT_ADDRESS));
} else {
  console.warn('[nft] BOP_REWARD_NFT_ADDRESS unset — /api/reward-nfts disabled');
}
if (evmAttestationReady()) {
  console.log(
    '[attest] EVM forum attestation ready — contract',
    getAddress(BOP_FORUM_ATTESTATION_ADDRESS),
    'relayer',
    attestationRelayer.address
  );
} else {
  console.warn(
    '[attest] EVM forum attestation unset — set ROBINHOOD_RPC_URL, BOP_FORUM_ATTESTATION_ADDRESS, ATTESTATION_RELAYER_PRIVATE_KEY'
  );
}
...
function parseFeePayerKeypairFromEnv() {
  ...
  if (!raw) return null;
  try {
    // A) JSON array (Solana CLI id.json / wallet export as bytes): [12,34,...]
    if (raw.startsWith('[')) {
...
function attestationFeePayerLabel() {
  if (evmAttestationReady()) return attestationRelayer.address;
  if (memoFeePayer) return memoFeePayer.publicKey.toBase58();
  return 'unknown';
}
...
async function parseLiteDepositFromTxSig(txSig) {
  ...
  /** Optional: restrict BOP pool deposits to this sender (public base58). Legacy: DIVIDENDS_DEV_WALLET. */
  const allowedSender = (
...
/** Solana Memo program: instruction data max 566 bytes (UTF-8). Legacy fallback only. */
const SOLANA_MEMO_MAX_BYTES = 566;

if (!evmAttestationReady()) {
  if (!memoFeePayer) {
    console.warn(
      '[memo] Legacy Solana memo unset (optional) — prefer EVM BopForumAttestation env vars'
    );
  } else {
    console.log(`[memo] legacy Solana fee payer: ${memoFeePayer.publicKey.toBase58()}`);
  }
}
...
async function sendEvmAttestation(authorWallet, memoText) {
  if (!evmAttestationReady()) {
    return { ok: false, tx_sig: null, fee_payer: null, error: 'EVM attestation not configured' };
  }
  const memo = String(memoText ?? '');
  const buf = Buffer.from(memo, 'utf8');
  const feePayer = attestationRelayer.address;
  if (buf.length < 1) {
    return { ok: false, tx_sig: null, fee_payer: feePayer, error: 'Empty memo' };
  }
  if (buf.length > EVM_ATTESTATION_MAX_MEMO_BYTES) {
    return {
      ok: false,
      tx_sig: null,
      fee_payer: feePayer,
      error: `Memo too long: ${buf.length} bytes (max ${EVM_ATTESTATION_MAX_MEMO_BYTES})`,
    };
  }
  let author;
  try {
    author = getAddress(String(authorWallet ?? ''));
  } catch {
    return {
      ok: false,
      tx_sig: null,
      fee_payer: feePayer,
      error: 'Invalid author wallet (need EVM 0x address)',
    };
  }
  try {
    const tx = await forumAttestationContract.attest(author, memo);
    const receipt = await tx.wait();
    const hash = receipt?.hash || tx.hash;
    return { ok: true, tx_sig: hash, fee_payer: feePayer, error: null };
  } catch (e) {
    const detail = e instanceof Error ? e.message : String(e);
    console.error('[attest] EVM send failed:', detail);
    return { ok: false, tx_sig: null, fee_payer: feePayer, error: detail };
  }
}

/** Legacy Solana Memo path (only used if EVM attestation is not configured). */
...
/** Prefer EVM BopForumAttestation; fall back to Solana Memo if still configured. */
async function sendOnchainAttestation({ author_wallet, memo }) {
  if (evmAttestationReady()) {
    return sendEvmAttestation(author_wallet, memo);
  }
  return sendMemoAttestation(memo);
}
...
async function queueOnchainAttestation(row) {
  ...
  try {
    const sent = await sendOnchainAttestation({
...
async function retryFailedOnchainAttestationsOnce(limit = 10) {
  try {
    const { data: rows, error } = await supabase
      .from('forum_onchain_attestations')
      .select('id, memo, attempts, author_wallet')
      .eq('status', 'failed')
...
/** Canonical EIP-55 checksum address. Throws if invalid. */
function canonicalizeWalletAddress(walletAddress) {
  return getAddress(String(walletAddress ?? '').trim());
}

function verifyWalletSignature(walletAddress, message, signatureBase64) {
  try {
    const expected = canonicalizeWalletAddress(walletAddress);
    const sig = uint8FromBase64(signatureBase64);
    // ECDSA personal_sign signatures are 65 bytes
    if (sig.length !== 65) return false;
    const sigHex = `0x${Buffer.from(sig).toString('hex')}`;
    const recovered = verifyMessage(message, sigHex);
    return canonicalizeWalletAddress(recovered) === expected;
  } catch {
    return false;
  }
}
...
function messageLooksValid(message, wallet) {
  ...
  return (
    message.includes('BOP forum registration') &&
...
function messageLooksLikeProfileUpdate(message, wallet) {
  if (!message || typeof message !== 'string') return false;
  return (
    message.includes('BOP profile update') &&
    message.includes(`Wallet: ${wallet}`) &&
...
function messageLooksLikeAvatarUpload(message, wallet) {
  if (!message || typeof message !== 'string') return false;
  return (
    message.includes('BOP avatar upload') &&
    message.includes(`Wallet: ${wallet}`) &&
    message.includes('Nonce:')
  );
}
...
function messageLooksLikeForumNewThread(message, wallet) {
  if (!message || typeof message !== 'string') return false;
  return (
    message.includes('BOP forum new thread') &&
    message.includes(`Wallet: ${wallet}`) &&
...
function messageLooksLikeForumThreadReply(message, wallet) {
  if (!message || typeof message !== 'string') return false;
  return (
    message.includes('BOP forum thread reply') &&
    message.includes(`Wallet: ${wallet}`) &&
    message.includes('Board:') &&
...
/**
 * Liteboard token gate: valid ERC-20 on Robinhood Chain (decimals() succeeds).
 * DB column remains `mint` (stores checksummed 0x address).
 */
async function resolveLiteboardMintCanonicalOrError(mintInput) {
  const raw = String(mintInput ?? '').trim();
  if (!isAddress(raw)) {
    return {
      ok: false,
      error: 'Invalid token contract address (expected 0x… EVM address on Robinhood Chain).',
    };
  }
  const mintCanonical = getAddress(raw);
  const provider = getEthProvider();
  if (!provider) {
    return {
      ok: false,
      error: 'ROBINHOOD_RPC_URL is not configured — cannot verify tokens on-chain.',
    };
  }
  try {
    ...
    return {
      ok: false,
      error:
        'Could not read this token on Robinhood Chain (not a valid ERC-20, or wrong network/RPC).',
    };
  }
}

async function fetchErc20DisplayMetadata(tokenAddress) {
  const empty = {
    token_name: null,
    token_symbol: null,
    usd_market_cap: null,
  ...
  const provider = getEthProvider();
  if (!provider || !isAddress(tokenAddress)) return empty;
  try {
    const token = new Contract(
    ...
    return {
      token_name: trimSplMetaDisplayString(name),
      token_symbol: trimSplMetaDisplayString(symbol),
      usd_market_cap: null,
      token_price_usd: null,
    };
  } catch {
...
/** @deprecated pump.fun gate removed — Liteboards use Robinhood ERC-20 verification. */
async function assertMintListedOnPumpFunForLiteboardDeploy(_mintCanon) {
  return { ok: true };
}
...
async function fetchSplMintDisplayMetadata(mintCanon) {
  ...
  // Robinhood / EVM ERC-20 path (primary for BOP).
  if (isAddress(String(mintCanon ?? ''))) {
    return fetchErc20DisplayMetadata(mintCanon);
  }
  let mintPk;
  try {
...
async function assertWalletIsMintCreator(walletBase58, mintInput) {
  let walletCanon;
  try {
    walletCanon = canonicalizeWalletAddress(walletBase58);
  } catch {
...
function liteboardTokenLine(message) {
  return message.match(/^(?:Mint|Token):\s*(.+)$/m);
}

/** Checksum EVM 0x address, or legacy Solana base58 mint. */
function canonicalizeLiteboardTokenId(raw) {
...
function messageLooksLikeLiteboardMintVerify(message, wallet) {
  return (
    (message.startsWith('BOP liteboard mint verify\n') ||
      message.startsWith('BOP liteboard token verify\n')) &&
    message.includes(`Wallet: ${wallet}`) &&
    Boolean(liteboardTokenLine(message)) &&
    message.includes('Nonce:')
  );
}

function parseLiteboardMintVerifyMessage(message) {
  const mintLine = liteboardTokenLine(message);
  if (!mintLine) return null;
  const mint = canonicalizeLiteboardTokenId(mintLine[1]);
  return mint ? { mint } : null;
}

function messageLooksLikeLiteboardCreate(message, wallet) {
  return (
    message.startsWith('BOP liteboard create\n') &&
    message.includes(`Wallet: ${wallet}`) &&
    Boolean(liteboardTokenLine(message)) &&
...
function messageLooksLikeLiteboardDelete(message, wallet) {
  return (
    message.startsWith('BOP liteboard delete\n') &&
    message.includes(`Wallet: ${wallet}`) &&
    Boolean(liteboardTokenLine(message)) &&
    message.includes('Nonce:')
  );
...
function messageLooksLikeLiteboardNewThread(message, wallet) {
  return (
    message.startsWith('BOP liteboard new thread\n') &&
    message.includes(`Wallet: ${wallet}`) &&
    Boolean(liteboardTokenLine(message)) &&
...
function messageLooksLikeLiteboardThreadReply(message, wallet) {
  return (
    message.startsWith('BOP liteboard thread reply\n') &&
    message.includes(`Wallet: ${wallet}`) &&
    Boolean(liteboardTokenLine(message)) &&
    message.includes('Channel:') &&
    message.includes('Thread number:') &&
...
function messageLooksLikeForumEditPost(message, wallet) {
  if (!message || typeof message !== 'string') return false;
  return (
    message.includes('BOP forum edit post') &&
    message.includes(`Wallet: ${wallet}`) &&
...
function messageLooksLikeForumPollCreate(message, wallet) {
  if (!message || typeof message !== 'string') return false;
  return (
    message.startsWith('BOP forum poll create\n') &&
    message.includes(`Wallet: ${wallet}`) &&
...
function messageLooksLikeForumPollVote(message, wallet) {
  if (!message || typeof message !== 'string') return false;
  return (
    message.startsWith('BOP forum poll vote\n') &&
    message.includes(`Wallet: ${wallet}`) &&
...
function messageLooksLikeForumPollEdit(message, wallet) {
  if (!message || typeof message !== 'string') return false;
  return (
    message.startsWith('BOP forum poll edit\n') &&
    message.includes(`Wallet: ${wallet}`) &&
    message.includes('Poll:') &&
    message.includes('Nonce:') &&
    message.includes('Mode:')
...
function parseForumPollEditMessage(message) {
  ...
  if (lines[0]?.trim() !== 'BOP forum poll edit') return null;
  let i = 1;
  let pollId = null;
  let mode = null;
...
function parseForumPollCreateMessage(message) {
  const lines = message.split('\n');
  if (lines[0]?.trim() !== 'BOP forum poll create') return null;
  let i = 1;
  let postId = null;
  let mode = 'single';
  for (; i < lines.length; i++) {
...
function parseForumPollVoteMessage(message) {
  const lines = message.split('\n');
  if (lines[0]?.trim() !== 'BOP forum poll vote') return null;
  let i = 1;
  let pollId = null;
  for (; i < lines.length; i++) {
    const L = lines[i];
...
function isGovernanceSectionName(section) {
  return String(section ?? '').trim().toUpperCase() === 'BOP GOVERNANCE';
}
...
async function hasGovernanceAccessForWallet(walletOk) {
  ...
  try {
    ...
    if (isOwnerWallet(walletOk) || prof?.is_admin === true) return true;
...
async function verifyIsAdmin(walletOk) {
  if (isOwnerWallet(walletOk)) return true;
  const { data: p } = await supabase
    .from('profiles')
    .select('is_admin')
    .eq('wallet', walletOk)
...
function adminSessionSecretBytes() {
  const raw =
    ...
    sha256Hex(String(SUPABASE_SERVICE_ROLE_KEY) + 'bop-admin-session-v1');
...
function adminSessionVerifyToken(token) {
  ...
  try {
    ...
    try {
      return canonicalizeWalletAddress(j.w);
    } catch {
      return null;
    }
  } catch {
...
function messageLooksLikeAdminSession(message, wallet) {
  return (
    ...
    message.includes('BOP admin session') &&
    message.includes(`Wallet: ${wallet}`) &&
    message.includes('Nonce:')
  );
}
...
async function fetchLiteHoldingsUi(walletAddress) {
  if (!BOP_TOKEN_ADDRESS) return '0';
  const provider = getEthProvider();
  if (!provider) return '0';
  try {
    const token = new Contract(BOP_TOKEN_ADDRESS, ERC20_BALANCE_ABI, provider);
    ...
    console.error('[bop] balanceOf UI failed:', e?.message || e);
    return '0';
  }
}

async function fetchLiteHoldingsRaw(walletAddress) {
  if (!BOP_TOKEN_ADDRESS) return 0n;
  const provider = getEthProvider();
  ...
  try {
    ...
    console.error('[bop] balanceOf raw failed:', e?.message || e);
    return 0n;
  }
}

/** $BOP UI amount assumed same units as 1B total supply (whole tokens). */
...
app.get('/api/lite-holdings', async (req, res) => {
  ...
  let walletOk;
  try {
    walletOk = canonicalizeWalletAddress(raw);
  } catch {
  ...
  try {
    ...
    return res.status(502).json({
      error: 'Could not read BOP balance from chain.',
      detail,
    });
  }
});
...
function messageLooksLikeDividendClaim(message, wallet, periodId) {
  ...
  return (
    message.includes('BOP dividend claim') &&
    message.includes(`Wallet: ${wallet}`) &&
...
// Submit a deposit tx signature (admin-only). Server verifies tx contains SPL transfer of BOP
...
app.post('/api/dividends/deposits/submit', async (req, res) => {
  ...
  if (!parsed) {
    return res.status(400).json({
      error:
        'Tx did not match expected BOP transfer (dev -> treasury). Submit the correct tx signature.',
    });
  }
...
/**
 * Owned BOP Reward NFTs for a wallet (current holdings + original mint time from Transfer mint).
 * GET /api/reward-nfts?wallet=0x…
 */
app.get('/api/reward-nfts', async (req, res) => {
  ...
  if (!BOP_REWARD_NFT_ADDRESS || !isAddress(BOP_REWARD_NFT_ADDRESS)) {
    return res.status(503).json({ error: 'BOP_REWARD_NFT_ADDRESS is not configured' });
  }
  const provider = getEthProvider();
  if (!provider) {
    return res.status(503).json({ error: 'ROBINHOOD_RPC_URL is not configured' });
  }

  const nftAddress = getAddress(BOP_REWARD_NFT_ADDRESS);
  try {
    ...
    const logs = await provider.getLogs({
      address: nftAddress,
      topics: [REWARD_NFT_TRANSFER_TOPIC],
      fromBlock: 0,
      toBlock: 'latest',
    });
    ...
    /** @type {Set<number>} */
    const owned = new Set();
    /** @type {Map<number, { blockNumber: number, txHash: string }>} */
    const mintMeta = new Map();

    for (const log of logs) {
      let parsed;
      try {
        parsed = REWARD_NFT_IFACE.parseLog(log);
      } catch {
        continue;
      }
      ...
      if (from === ZeroAddress) {
        mintMeta.set(tokenId, {
          blockNumber: Number(log.blockNumber),
          txHash: String(log.transactionHash || ''),
        });
      }
      if (to.toLowerCase() === wallet.toLowerCase()) owned.add(tokenId);
      if (from.toLowerCase() === wallet.toLowerCase()) owned.delete(tokenId);
    }

    const tokenIds = [...owned].sort((a, b) => a - b);
    const blockNums = [
      ...new Set(
        tokenIds
          .map((id) => mintMeta.get(id)?.blockNumber)
    ...
    await Promise.all(
      blockNums.map(async (bn) => {
        try {
          ...
          if (block?.timestamp != null) blockTs.set(bn, Number(block.timestamp));
        } catch {
          /* ignore */
        }
      })
    ...
    const nfts = tokenIds.map((tokenId) => {
      const meta = mintMeta.get(tokenId);
      const mintedAtUnix = meta ? blockTs.get(meta.blockNumber) ?? null : null;
      return {
        token_id: tokenId,
    ...
    console.error('[reward-nfts]', e);
    return res.status(500).json({
      error: e instanceof Error ? e.message : 'Failed to load reward NFTs',
    });
  }
});
...
app.get('/api/dividends/status', async (req, res) => {
  ...
  if (walletRaw) {
    try {
      walletOk = canonicalizeWalletAddress(walletRaw);
    } catch {
      return res.status(400).json({ error: 'Invalid wallet' });
    }
  }
  ...
  if (treasuryWalletPublic && LITE_TOKEN_MINT) {
    try {
      ...
      console.error('[dividends] treasury on-chain BOP balance failed', e);
    }
  }
...
app.post('/api/dividends/claims/claim', async (req, res) => {
  ...
  let walletOk;
  try {
    walletOk = canonicalizeWalletAddress(wallet);
  } catch {
    return res.status(400).json({ error: 'Invalid wallet' });
  }
  ...
  let userPk;
  try {
    userPk = new PublicKey(walletOk);
  } catch {
    // Claim payouts still use Solana SPL transfers; EVM payouts are not wired yet.
    ...
    return res.status(400).json({
      error:
        'Dividend payouts are not available for EVM wallets yet. Forum login/signing works with MetaMask; on-chain claims will follow the Robinhood Chain migration.',
    });
  }
...
app.post('/api/register', async (req, res) => {
  ...
  const { error: insertError } = await supabase.from('profiles').insert({
    wallet: walletOk,
    username: u,
    ...(isOwnerWallet(walletOk) ? { is_admin: true } : {}),
  });
...
app.get('/api/profile', async (req, res) => {
  ...
  let row = { ...data, is_moderator: data.is_moderator === true };
  if (isOwnerWallet(walletOk)) {
    row = { ...row, is_admin: true };
    if (data.is_admin !== true) {
      void supabase.from('profiles').update({ is_admin: true }).eq('wallet', walletOk);
    }
  }
...
function messageLooksLikePmKeyRegister(message, wallet, encPublicKey) {
  return (
    typeof message === 'string' &&
    message.includes('BOP PM key register') &&
    message.includes(`Wallet: ${wallet}`) &&
...
function pmSessionSecretBytes() {
  const raw =
    ...
    sha256Hex(String(SUPABASE_SERVICE_ROLE_KEY) + 'bop-pm-session-v1');
  return crypto.createHash('sha256').update(raw, 'utf8').digest();
}
...
function messageLooksLikePmSession(message, wallet) {
  return (
    typeof message === 'string' &&
    message.includes('BOP PM session') &&
    message.includes(`Wallet: ${wallet}`) &&
    message.includes('Nonce:')
  );
}
...
app.post('/api/pm/send', async (req, res) => {
  ...
  try {
    walletOk = canonicalizeWalletAddress(wallet);
    recipientOk = canonicalizeWalletAddress(recipient_wallet);
  } catch {
    return res.status(400).json({ error: 'Invalid wallet' });
  }
  ...
  const { data: inserted, error: insErr } = await supabase
    ...
    .insert({
      ...
      memo: 'pending',
      fee_payer: attestationFeePayerLabel(),
      status: 'pending',
      attempts: 0,
      last_error: null,
      tx_sig: null,
...
function messageLooksLikeProfileSocialUpdate(message, wallet) {
  if (!message || typeof message !== 'string') return false;
  return (
    message.includes('BOP profile socials update') &&
    message.includes(`Wallet: ${wallet}`) &&
...
function parseBopCompactMemo(memo) {
  const raw = String(memo ?? '');
  const parts = raw.split('|');
  if (parts[0] !== 'v1') {
    return { ok: false, error: 'Not a BOP v1 pipe memo', raw };
  }
  const kind = parts[1];
  if (kind === 'tc') {
    if (parts.length < 10) {
    ...
    return {
      ...
      rows: [
        ...
        { idx: 10, label: 'BOP holdings snapshot (UI)', value: parts[9] || '—' },
      ],
    };
  }
  if (kind === 'rp') {
  ...
  if (kind === 'pv') {
    ...
    return {
      ...
      rows: [
        ...
        { idx: 9, label: 'BOP holdings snapshot (UI)', value: parts[8] || '—' },
      ],
    };
  }
  if (kind === 'pm') {
  ...
  if (kind === 'pc') {
    ...
    return {
      ...
      rows: [
        ...
        { idx: 11, label: 'BOP holdings snapshot (UI)', value: parts[10] || '—' },
      ],
    };
  }
  if (kind === 'pb') {
...
const FORUM_ATTEST_DECODE_IFACE = new Interface([
  'function attest(address author, string memo)',
  'event ForumAttested(address indexed relayer, address indexed author, string memo)',
]);
...
/** Decode Robinhood Chain attestation tx (BopForumAttestation.attest / ForumAttested). */
async function decodeEvmForumAttestationTx(txHash) {
  const provider = getEthProvider();
  if (!provider) {
    return { error: 'ROBINHOOD_RPC_URL is not configured' };
  }
  ...
  if (!tx || !receipt) {
    return { error: 'Transaction not found on Robinhood Chain RPC' };
  }

  const memos = [];
  try {
    const decoded = FORUM_ATTEST_DECODE_IFACE.parseTransaction({
      data: tx.data,
      value: tx.value ?? 0n,
    });
    if (decoded?.name === 'attest') {
      const memo = String(decoded.args?.memo ?? decoded.args?.[1] ?? '');
      if (memo) memos.push(memo);
    }
  } catch {
    /* not an attest() call — may still have ForumAttested logs */
  }

  for (const log of receipt.logs ?? []) {
    try {
      const parsedLog = FORUM_ATTEST_DECODE_IFACE.parseLog({
        topics: log.topics,
        data: log.data,
      });
      if (parsedLog?.name === 'ForumAttested') {
        const memo = String(parsedLog.args?.memo ?? '');
        if (memo && !memos.includes(memo)) memos.push(memo);
      }
    } catch {
  ...
  try {
    ...
    if (block?.timestamp != null) blockTime = Number(block.timestamp);
  } catch {
    blockTime = null;
  }
  ...
  return {
    ...
    blockTime,
    feePayer: tx.from ?? null,
    author: (() => {
      try {
        const d = FORUM_ATTEST_DECODE_IFACE.parseTransaction({
          data: tx.data,
          value: tx.value ?? 0n,
        });
        if (d?.name === 'attest') {
          return getAddress(String(d.args?.author ?? d.args?.[0]));
        }
      } catch {
        /* ignore */
      }
    ...
    memos,
    rpcUsed: ROBINHOOD_RPC_URL,
    rpcLabel: 'Robinhood Chain',
    commitment: receipt.status === 1 ? 'success' : 'reverted',
    networkMode: 'robinhood',
    chain: 'evm',
  };
}

/**
 * Decode an attestation tx:
 * - 0x… hash → Robinhood Chain EVM (BopForumAttestation)
 * - otherwise → legacy Solana Memo path
 */
app.get('/api/forum/decode-memo-tx', async (req, res) => {
  ...
  // Primary path: Robinhood / EVM attestation txs
  if (isEvmTxHash(signature)) {
    try {
      const decoded = await decodeEvmForumAttestationTx(signature);
      if (decoded.error) {
        return res.status(404).json({ error: decoded.error, networkMode: 'robinhood' });
      }
      if (!decoded.memos?.length) {
        return res.status(422).json({
          error:
            'Transaction found, but no BopForumAttestation attest()/ForumAttested memo was found in input or logs.',
          signature,
      ...
      const parsed = decoded.memos.map((m) => parseBopCompactMemo(m));
      return res.json({
        ...
        memos: decoded.memos,
        parsed,
        rpcUsed: decoded.rpcUsed,
        rpcLabel: decoded.rpcLabel,
      ...
      console.error(e);
      return res.status(500).json({
        error: e instanceof Error ? e.message : 'Failed to decode EVM transaction',
      });
    }
  }

  // Legacy Solana Memo path
  try {
    ...
    if (!tx) {
      return res.status(404).json({
        error:
          'Transaction not found. For Robinhood attestations, paste a 0x… transaction hash. ' +
          'Legacy Solana memos still work with a base58 signature.',
        networkMode,
      });
    }
    ...
    const parsed = memos.map((m) => parseBopCompactMemo(m));
    ...
    return res.json({
      ...
      networkMode,
      chain: 'solana',
      explorerCluster: onDevnet ? 'devnet' : 'mainnet',
    });
  } catch (e) {
    console.error(e);
    return res.status(500).json({
...
function messageLooksLikePostVote(message, wallet, postId, action) {
  ...
  return (
    message.includes('BOP forum post vote') &&
    message.includes(`Wallet: ${wallet}`) &&
...
app.get('/api/forum/post-votes', async (req, res) => {
  ...
  if (w) {
    try {
      walletOk = canonicalizeWalletAddress(w);
    } catch {
      return res.status(400).json({ error: 'Invalid wallet' });
    }
  }
...
app.post('/api/forum/polls', async (req, res) => {
  ...
  if (isGovernanceSectionName(boardPoll.section)) {
    ...
    if (!allowGovPoll) {
      return res.status(403).json({
        error:
          'Governance board polls require >= 0.25% supply holdings (2,500,000 BOP), or admin/moderator.',
      });
    }
  }
...
app.post('/api/forum/poll-ballots', async (req, res) => {
  ...
  let walletOkPb;
  try {
    walletOkPb = canonicalizeWalletAddress(wallet);
  } catch {
    return res.status(400).json({ error: 'Invalid wallet' });
  }
...
app.get('/api/forum/boards', async (req, res) => {
  ...
  if (walletQ) {
    try {
      walletOk = canonicalizeWalletAddress(walletQ);
    } catch {
      return res.status(400).json({ error: 'Invalid wallet' });
    }
  }
  ...
  if (hasGovernance) {
    ...
    if (!allowGovernance) {
      if (isGovernanceSectionName(section)) {
        return res.status(403).json({
          error:
            'Governance section requires >= 0.25% supply holdings (2,500,000 BOP), or admin/moderator.',
        });
      }
...
app.get('/api/forum/boards/:slug/threads', async (req, res) => {
  ...
  if (isGovernanceSectionName(board.section)) {
    ...
    if (!allowGovernance) {
      return res.status(403).json({
        error:
          'Governance board requires >= 0.25% supply holdings (2,500,000 BOP), or admin/moderator.',
      });
    }
  }
...
function messageLooksLikeAdminBoardUpdate(message, wallet) {
  return (
    typeof message === 'string' &&
    message.includes('BOP admin board update') &&
    message.includes(`Wallet: ${wallet}`) &&
    message.includes('Board:') &&
...
function messageLooksLikeAdminUserSearch(message, wallet) {
  return (
    typeof message === 'string' &&
    message.includes('BOP admin user search') &&
    message.includes(`Wallet: ${wallet}`) &&
...
function messageLooksLikeAdminUserPatch(message, wallet) {
  return (
    typeof message === 'string' &&
    message.includes('BOP admin user patch') &&
    message.includes(`Wallet: ${wallet}`) &&
...
function messageLooksLikeAdminBan(message, wallet) {
  return (
    typeof message === 'string' &&
    message.includes('BOP admin ban user') &&
    message.includes(`Wallet: ${wallet}`) &&
    message.includes('Target wallet:') &&
...
function messageLooksLikeAdminDeletePost(message, wallet) {
  return (
    typeof message === 'string' &&
    message.includes('BOP admin delete post') &&
    message.includes(`Wallet: ${wallet}`) &&
    message.includes('Post id:') &&
    message.includes('Nonce:')
  );
...
app.post('/api/admin/liteboard/grant', async (req, res) => {
  ...
  if (!prof) {
    return res.status(400).json({
      error: 'owner_wallet must be a registered profile (BOP account).',
    });
  }
...
/** --- Liteboards (per-token mini forums on Robinhood Chain; see for_developers/sql/021_liteboards.sql) --- */
...
app.post('/api/liteboard/verify-mint', async (req, res) => {
  ...
  const authCheck = await resolveLiteboardMintCanonicalOrError(parsed.mint);
  if (!authCheck.ok) {
    return res.status(403).json({ error: authCheck.error });
  }
  // Legacy Solana-only strict creator proof (ignored for EVM ERC-20 tokens).
  if (LITEBOARD_STRICT_CREATOR_VERIFY && !isAddress(authCheck.mint)) {
    const creator = await assertWalletIsMintCreator(walletOk, authCheck.mint);
    if (!creator.ok) {
      return res.status(403).json({ error: creator.error });
    }
  }
  ...
  if (taken?.id) {
    return res.status(409).json({
      error: 'A Liteboard already exists for this token.',
    });
  }
...
app.post('/api/liteboard/create', async (req, res) => {
  ...
  if (!profC) {
    return res.status(403).json({
      error: 'Register a BOP profile before creating a Liteboard.',
    });
  }
  const authCheck2 = await resolveLiteboardMintCanonicalOrError(parsed.mint);
  if (!authCheck2.ok) {
    return res.status(403).json({ error: authCheck2.error });
  }
  if (LITEBOARD_STRICT_CREATOR_VERIFY && !isAddress(authCheck2.mint)) {
    const creator2 = await assertWalletIsMintCreator(walletOk, authCheck2.mint);
    if (!creator2.ok) {
      return res.status(403).json({ error: creator2.error });
    }
  }
...
(async () => {
  if (DEV_VITE) {
    ...
    httpServer.listen(LISTEN_PORT, LISTEN_HOST, () => {
      console.log(
        `BOP dev (Vite + API) on http://127.0.0.1:${LISTEN_PORT} — open this URL in the browser`
      );
    });
    ...
    app.use((req, res) => {
      res.status(404).type('text').send('BOP API — only /api/* routes');
    });
    app.listen(LISTEN_PORT, LISTEN_HOST, () => {
      console.log(`BOP API listening on http://${LISTEN_HOST}:${LISTEN_PORT}`);
    });
    setInterval(() => void retryFailedOnchainAttestationsOnce(10), 30_000);
    setInterval(() => void finalizeDividendsPeriodsOnce(2), 30_000);
  }
